...
import color
from color import string_to_rgb, string_to_hsv, hsv_to_string
...
class LedController:
    ...
    def __init__(self, preferences=None, pin_red=22, pin_green=27, pin_blue=17):
        ...
        self.old_color = "0 0 0"    # The last 'selected' color, discounting any changes made by interpolation. i.e. the previous target_color
        self.previous_color = "0 0 0" # The last color that was actually sent to the lights
    ...
    def set_color(self, rgb_string):
        if self.preferences.color_change_interpolate:
            ...
            if interpolated_result == rgb_string:
                old_color = rgb_string
            rgb_string = interpolated_result
            
        rgb_string = self._apply_restrictions(rgb_string)
        
        if rgb_string == self.previous_color:
            # Sending repeated commands to the lights can cause flickering so we return here if the color has not changed
            return
    ...
    def _apply_restrictions(self, rgb_string):
        ...
        v = int(min(self.preferences.max_brightness / 100.0 * 255.0, max(v, self.preferences.min_brightness / 100.0 * 255.0)))
    ...
    def _morph_to_color(self, rgb_string):
        ...
        progress = (now - self.color_change_time).total_seconds() / self.preferences.color_change_duration
        if progress > 1:
