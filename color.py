...
def morph(from_string, to_string, t):
    fh, fs, fv = string_to_hsv(from_string)   # from_hue, from_saturation, from_value
    th, ts, tv = string_to_hsv(to_string)         # to_hue, to_saturation, to_value
    
    # Decide how close the hues are to each other to determine whether
    # we should interpolate directly between them or use an intermediate fade-out
    distance = fabs(fh - th)
    direct = distance < 0.2
