...
from util import read_line, read_lines, safe_load
...
class Behaviour:
    ...
    MIN_BEAT_DURATION = 0.25 # Restrict any bpm-based behaviours to a minimum beat duration (in seconds) to prevent unpleasant flickering/strobing
    ...
    # Return the modified color as a string, and True if this modified color should be considered as canonical
    # (i.e. if True, this behaviour will affect AI learning behaviour, False will not)
...
class CycleBehaviour(Behaviour):
    ...
    def update(self, fallback_color, now):
        ...
        delta = ((now - self.cycle_start).total_seconds() / self.duration) % 1.0
...
class DiscoBehaviour(Behaviour):
    ...
    def to_string(self):
        return "DiscoBehaviour[bpm:{}, color_duration:{}]".format(self.bpm, self.color_duration)
...
### WARNING WARNING WARNING WARNING WARNING WARNING WARNING WARNING WARNING WARNING WARNING WARNING WARNING
###
### This behaviour causes the lights to flicker which may cause nausea or siezure
### Please do not enable this feature if you or anyone around you is prone to siezure
### through photosensitive epilepsy (https://en.wikipedia.org/wiki/Photosensitive_epilepsy)
### or any similar condition.
###
### This behaviour is intended to simulate the lightning effects from old-timey horror movies
### 
'''
class SpookyBehaviour(Behaviour):
    ...
    def update(self, fallback_color, now):
        ...
        sleep(0.5) # This can be removed or reduced if you are sure that nobody nearby
...
class AIBehaviour(Behaviour):
    ...
    def update(self, fallback_color, now):
        ...
        if color != "":
            self.color = color  # Cache a copy of the color to prevent flickering in case of file read errors
...
#                    print("brightness={}".format(color.get_brightness(fallback_color)))
                    raise Exception("Not using mech color - not dark enough just now")
                self.color = mech_color
#                print("MECH {}".format(mech_color))
                return mech_color, False
            else:
                self.color = ""
#                print("Using mech color '{}' [{}s]".format(color, int(now.timestamp() - timestamp)))
...
class PulseBehaviour(Behaviour):
    ...
    def update(self, fallback_color, now):
        ...
        delta = ((now - self.cycle_start).total_seconds() / self.beat_duration) % 1.0
